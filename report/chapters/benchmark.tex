...
\((\text{pallet}, \text{pack}, \text{grip})\) combination defined by the
benchmark grid. The pallet presets are listed in
Table~\ref{tab:bench-pallets}, while the pack sizes are listed in
...
\texttt{cp-sat} uses a SAT-based Boolean/integer representation, and
...
\begin{table}[h]
  ...
  \begin{minipage}[t]{0.49\linewidth}
    ...
    \begin{tabularx}{0.8\linewidth}{X}
      \toprule\arraybackslash\textbf{Pack size (mm)} \\
      \midrule
      ...
      \bottomrule
    \end{tabularx}
    \caption{Pack sizes used in the benchmark.}
