...
The second constraint makes the chunks consecutive. For each chunk \(g\), the
next chunk starts immediately after it, since the start position of chunk
\(g+1\) is obtained by adding the length of chunk \(g\) to
\texttt{chunk\_start[g]}. This prevents gaps and overlaps between adjacent
chunks.
