...
Some small Python scripts are built around the MiniZinc model, which
il loaded through \texttt{minizinc-python} and attached
to the Gecode solver:
...
The Python scripts assigns concrete values to the MiniZinc parameters
and starts the solve:
...
After solving, Python read the MiniZinc output arrays
\texttt{group\_start}, \texttt{group\_len}, \texttt{drop\_order}, and
\texttt{plate} to get the pallet layout and schedule.
...
\text{pack}, \text{grip})$ combination from the lists in Table~\ref{tab:bench-grid},
against diffenrent solvers ("gecode", "cp-sat", "chuffed" and "highs", that uses
different representation for the CSP problem, namely ..., with a per-run
...
\begin{table}[h]
  ...
  \begin{tabular}{l}
    \toprule
    \textbf{Grip multiplier} \\
    \midrule
    $1 \cdot \texttt{boxes\_x\_dim}$ \\
    $2 \cdot \texttt{boxes\_x\_dim}$ \\
    $3 \cdot \texttt{boxes\_x\_dim}$ \\
    \bottomrule
  \end{tabular}
  \caption{Pallet presets, pack sizes, and gripper sizes used in the
  benchmark.}
  \label{tab:bench-grid}
\end{table}

The gripper size is varied across the integer multiples
$S \in \{1,2,3\} \cdot \texttt{boxes\_x\_dim}$, in order to
study the effect of the gripper size.

Figure~\ref{fig:scaling-overall} shows solve time with respect to the
total number of boxes on the pallet for every solver across all
instances. The arithmetic mean and min/max range per solver is
summarised in Figure~\ref{fig:solver-summary}.
...
\begin{figure}[h]
  ...
  \caption{Per-solver mean of solve time (bar) with min/max
...
\subsection{Effect of gripper size}

Sweeping the gripper multiplier $k \in \{1,2,3\}$ changes
\texttt{max\_pkble\_boxes} and therefore the total number of groups for a
given pallet/pack pair. Smaller grippers force more groups per row,
which tightens the partition but lengthens the schedule; larger
grippers reduce the number of groups but loosen the row-partition
constraint. Figure~\ref{fig:scaling-by-grip} reports the mean of solve
time as a function of $k$, with one line per solver.

%\begin{figure}[h]
%  \centering
%  \includegraphics[width=0.7\linewidth]{images/scaling_by_grip.pdf}
%  \caption{Mean of solve time vs.\ gripper multiplier $k$, with
%  $S = k \cdot \texttt{boxes\_x\_dim}$, aggregated across all
%  pallet/pack instances.}
%  \label{fig:scaling-by-grip}
%\end{figure}
...
\begin{figure}[h]
  ...
  zero overhead; all points lie above it.}
