from pathlib import Path

import matplotlib.pyplot as plt
import matplotlib.patches as patches
from matplotlib import cm


def _pack_xy(pack_idx, packs_along_x, cell_w, cell_h):
    col = (pack_idx - 1) % packs_along_x
    row = (pack_idx - 1) // packs_along_x
    return col * cell_w, row * cell_h


def _group_color(g_idx):
    return cm.tab20(g_idx % 20)


def _draw_packs(ax, groups, placed_set, current_idx, packs_along_x, cell_w, cell_h):
    for g_idx, group in enumerate(groups):
        for pack in group:
            x, y = _pack_xy(pack, packs_along_x, cell_w, cell_h)
            if g_idx in placed_set or g_idx == current_idx:
                color = _group_color(g_idx)
                alpha = 0.75
            else:
                color = "lightgray"
                alpha = 0.25
            ax.add_patch(patches.Rectangle(
                (x, y), cell_w, cell_h,
                linewidth=0.6, edgecolor="black", facecolor=color, alpha=alpha
            ))


def _group_bbox(group, packs_along_x, cell_w, cell_h):
    first, last = group[0], group[-1]
    x0, y0 = _pack_xy(first, packs_along_x, cell_w, cell_h)
    x1, _  = _pack_xy(last,  packs_along_x, cell_w, cell_h)
    return x0, y0, (x1 + cell_w) - x0, cell_h


def render_pallet(solution, out_path):
    """Render the full pallet, colored by group, with group labels."""
    px = solution["packs_along_x"]
    py = solution["packs_along_y"]
    cw = solution["cell_w"]
    ch = solution["cell_h"]
    groups = solution["groups"]

    width_in  = 10
    height_in = max(3.0, width_in * (py * ch) / (px * cw))
    fig, ax = plt.subplots(figsize=(width_in, height_in))

    _draw_packs(ax, groups, placed_set=set(range(len(groups))),
                current_idx=-1, packs_along_x=px, cell_w=cw, cell_h=ch)

    for g_idx, group in enumerate(groups):
        x0, y0, w, h = _group_bbox(group, px, cw, ch)
        ax.add_patch(patches.Rectangle(
            (x0, y0), w, h, linewidth=2.0, edgecolor="black", facecolor="none"
        ))
        ax.text(x0 + w/2, y0 + h/2, f"G{g_idx + 1}",
                ha="center", va="center", fontsize=11, fontweight="bold")
        for pack in group:
            x, y = _pack_xy(pack, px, cw, ch)
            ax.text(x + cw*0.92, y + ch*0.92, str(pack),
                    ha="right", va="bottom", fontsize=6, color="black", alpha=0.6)

    ax.set_xlim(-cw*0.1, px * cw + cw*0.1)
    ax.set_ylim(-ch*0.1, py * ch + ch*0.1)
    ax.invert_yaxis()
    ax.set_aspect("equal")
    ax.set_title(f"Pallet layout — {len(groups)} groups, {sum(len(g) for g in groups)} packs")
    ax.set_xticks([]); ax.set_yticks([])
    plt.tight_layout()
    plt.savefig(Path(out_path).with_suffix(".pdf"), format="pdf", dpi=600)
    plt.close(fig)


def render_schedule(solution, out_path):
    """Render the step-by-step drop schedule as one PDF per step."""
    px = solution["packs_along_x"]
    py = solution["packs_along_y"]
    cw = solution["cell_w"]
    ch = solution["cell_h"]
    groups   = solution["groups"]
    schedule = solution["schedule"]
    n_steps  = len(schedule)

    cell_aspect = (py * ch) / (px * cw)
    sub_w = 6.0

    base = Path(out_path).with_suffix("")
    base.parent.mkdir(parents=True, exist_ok=True)
    paths = []

    placed = set()
    for step in schedule:
        cur_g = step["group_index"] - 1

        fig, ax = plt.subplots(figsize=(sub_w, sub_w * cell_aspect))
        _draw_packs(ax, groups, placed, cur_g, px, cw, ch)

        x0, y0, w, h = _group_bbox(groups[cur_g], px, cw, ch)
        ax.add_patch(patches.Rectangle(
            (x0, y0), w, h, linewidth=2.5, edgecolor="red", facecolor="none"
        ))
        arrow_len = max(cw, ch) * 0.9
        if step["open_plate"] == "left":
            ax.annotate("", xy=(x0 - arrow_len, y0 + h/2),
                        xytext=(x0, y0 + h/2),
                        arrowprops=dict(arrowstyle="->", color="blue", lw=2.2))
        else:
            ax.annotate("", xy=(x0 + w + arrow_len, y0 + h/2),
                        xytext=(x0 + w, y0 + h/2),
                        arrowprops=dict(arrowstyle="->", color="blue", lw=2.2))

        ax.set_xlim(-cw, px * cw + cw)
        ax.set_ylim(-ch*0.2, py * ch + ch*0.2)
        ax.invert_yaxis()
        ax.set_aspect("equal")
        ax.set_title(f"Step {step['step']} of {n_steps} — G{cur_g + 1} ({step['open_plate']})", fontsize=11)
        ax.set_xticks([]); ax.set_yticks([])

        plt.tight_layout()
        step_path = base.with_name(f"{base.name}_step_{step['step']:02d}.pdf")
        plt.savefig(step_path, format="pdf", dpi=600)
        plt.close(fig)
        paths.append(step_path)

        placed.add(cur_g)

    return paths
