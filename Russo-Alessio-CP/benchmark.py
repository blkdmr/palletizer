...
def available_solvers(names):
    """Return only the solvers that are installed and reachable."""
    found = []
    for name in names:
        try:
            Solver.lookup(name)
...
def run_one(model, solver_name, box_x, box_y, pallet_x, pallet_y, grip_multiplier, timeout_s):
    """Solve a single instance and return a result record."""
...
def main():
    ...
    results = []
    n_instances = len(BOX_SIZES) * len(PALLET_PRESETS)
    ...
    out_file = root_dir / "results.json"
